...
def _leere_figur(text: str):
    ...
    ax.text(
        ...
        color=config.TEXT_GEDIMMT, fontsize=11, transform=ax.transAxes,
    )
    ax.axis("off")
    return fig
...
def create_chart_tagesverlauf(df: pd.DataFrame):
    ...
    ax.fill_between(x, df_heute["pv_erzeugung_kw"], alpha=0.20,
                    color=config.FARBE_UEBERSCHUSS, zorder=1)
    ax.fill_between(x, df_heute["netz_wert_kw"], alpha=0.15,
                    color=config.FARBE_DEFIZIT, zorder=1)
    ax.plot(x, df_heute["pv_erzeugung_kw"], color=config.FARBE_UEBERSCHUSS,
            linewidth=2.0, marker="o", markersize=2.5, label="Erzeugung (kW)", zorder=3)
    ax.plot(x, df_heute["netz_wert_kw"], color=config.FARBE_DEFIZIT,
            linewidth=2.0, marker="o", markersize=2.5, label="Verbrauch (kW)", zorder=3)
    ...
    ax.set_title("Tagesverlauf heute: Erzeugung & Verbrauch",
                 color="white", fontsize=9, fontweight="bold", pad=10)

    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.legend(facecolor=config.PANEL_BG, edgecolor="none", labelcolor="white",
